> React Native port note: the following is the original gpu-time model card. This port uses its unchanged weights on the CPU. French, Spanish, and Simplified Chinese support comes from separate deterministic language packs, not from retraining this model. The browser GPU benchmarks below do not describe mobile performance.
