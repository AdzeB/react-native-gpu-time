# React Native port attribution

This project includes MIT-licensed parser code, trained model weights, regression fixtures, and presentation helpers from gpu-time by Arik Chakma. See UPSTREAM.json for the pinned source commit. The mobile build additionally uses Babel and its Unicode-property-regex transform as development dependencies; no Babel runtime is shipped.

The original upstream notices follow. References to upstream training, website, benchmark, and video tools describe that project and do not imply that those tools are shipped in this package.
