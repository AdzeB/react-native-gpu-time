import { clonePlain } from "./runtime.js";
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    switch (token.label) {
      ...
      case Role.WEEKDAY: {
        ...
        if (pendingDayRange) {
          let position = weekdays.indexOf(days[days.length - 1]!);
...
function compileClause(input: Token[], diagnostics: Diagnostic[]): Clause {
  const last = input.slice().reverse().find((token) => token.label !== Role.O);
...
function splitExpressions(tokens: Token[]): Token[][] {
  ...
  return expressions.filter((expression) => {
    ...
    while (
      expression.length &&
      [Role.O, Role.GLUE, Role.JOIN].includes(expression[expression.length - 1]!.label)
    )
      expression.pop();
  ...
  if (separator < 0) {
    ...
    for (let index = 0; index < tokens.length; index++) {
      ...
      if (
        ...
        groups[groups.length - 1]!.some(isClock) &&
